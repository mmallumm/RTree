...
bool rectangle::areCrossed(const rectangle &other) const {
...
rectangle Irt_node::_getGeneralRec() const {
...
bool Irt_node::operator==(const Irt_node &other) const {
    return (this->_getGeneralRec() == other._getGeneralRec());
}
...
double Irt_node::_getOverlappingSize(std::vector<Irt_node *> &nodes) const {
    ...
    for (auto &node: nodes) {
        if (this != node) {
            overlappingSize += this->_getGeneralRec().getOverlapSizeWith(node->_getGeneralRec());
        }
    }
    return overlappingSize;
}

double Irt_node::_try_getNewOverlappingSize(std::vector<Irt_node *> &nodes, rectangle toExpand) const {
    rectangle expandedCurr = this->_getGeneralRec()._tryExtendTo(toExpand);

    double overlappingSize = 0;
    for (auto &node: nodes) {
        if (this != node) {
            overlappingSize += expandedCurr.getOverlapSizeWith(node->_getGeneralRec());
        }
    }
    return overlappingSize;
}
...
void r_tree::clear() {
    ...
    //labda deepSearch
    std::function< void(Irt_node*) > deepSearchClear = [&deepSearchClear] ( Irt_node* curr_I ) {
        //recursion base
        if(curr_I->_isLeaf()) {
            delete curr_I;
            return;
...
splitNodes r_tree::_splitLeaf(Irt_node* nodeI) {
    ...
    toAssert_rec.erase(std::remove(toAssert_rec.begin(), toAssert_rec.end(), r1), toAssert_rec.end());
    toAssert_rec.erase(std::remove(toAssert_rec.begin(), toAssert_rec.end(), r2), toAssert_rec.end());

    //cleaf from toAssert_data
    toAssert_data.erase(std::remove(toAssert_data.begin(), toAssert_data.end(), d1), toAssert_data.end());
    toAssert_data.erase(std::remove(toAssert_data.begin(), toAssert_data.end(), d2), toAssert_data.end());
    ...
    while(!toAssert_data.empty()) {
        ...
        toAssert_rec.erase(std::remove(toAssert_rec.begin(), toAssert_rec.end(), r), toAssert_rec.end());
        toAssert_data.erase(std::remove(toAssert_data.begin(), toAssert_data.end(), d), toAssert_data.end());
    }
    ...
    for(int i = 0; i < rec_v.size(); ++i) {
        for(int j = 0; j < rec_v.size(); ++j) {
            if(i != j) {
                ...
                double wasteArea = paired.area() - rec_v.at(i).area() - rec_v.at(j).area();
...
std::vector<void*> r_tree::search(const rectangle searchArea) const {
    ...
    std::function< void( Irt_node*, const rectangle, std::vector<void*>& ) > deepSearchData =
        [&deepSearchData, this] ( Irt_node* curr_I, const rectangle searchArea, std::vector<void*> &toReturn) {

        //recursion base
        if(curr_I->_isLeaf()) {
            leaf_rt_node* curr = dynamic_cast<leaf_rt_node*>(curr_I);
            auto ac = curr->child_mbr_v;
            auto ad = curr->data_v; 
            for(int i = 0; i < curr->child_mbr_v.size(); ++i) {
                if(ac.at(i).areCrossed(searchArea)) {
                    toReturn.push_back( ad.at(i) );
                }
            }
        ...
        for(auto &child : curr->child_v) {
            if( child->mbr.areCrossed(searchArea) ) {
                deepSearchData(child, searchArea, toReturn);
            }
        }
    };

    std::vector<void*> toReturn;
    deepSearchData(root, searchArea, toReturn);
