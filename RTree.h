...
public:
    ...
    bool areCrossed(const rectangle& other) const;
...
protected:
    ...
    rectangle _getGeneralRec() const;
    ...
    double _try_getNewOverlappingSize(std::vector<Irt_node*> &nodes, rectangle toExpand) const;
...
//r-tree------------
class r_tree {  //r*_tree realization
    Irt_node *root = nullptr;
    size_t height = 0;
    const size_t min_child_num;
    const size_t branch_fctr;  //branch_factor
